<%@ page language="java" contentType="text/html; charset = UTF-8"
	pageEncoding="UTF-8"%>

<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Chỉnh sửa TT sản phẩm</title>
<link rel="stylesheet"
	href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css"
	integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh"
	crossorigin="anonymous">
</head>
<body>
	<div class="container">
		<hr>
		<div class="row">

			<!-- edit form column -->
			<div class="col-md-9 personal-info">
				<h3>Thông tin sản phẩm</h3>

				<form class="form-horizontal" role="form"
					action="EditProductUkulele" method="post">
					<div class="form-group">
						<label class="col-lg-3 control-label">Mã sản phẩm:</label>
						<div class="col-lg-8">
							<input class="form-control" type="text" name="maSanPham"
								placeholder="Nhập mã sản phẩm" required="required"
								value="<%ServletContext context = request.getServletContext();
out.print(context.getAttribute("maSanPham"));%>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">Tên sản phẩm:</label>
						<div class="col-lg-8">
							<input class="form-control" type="text" name="tenSanPham"
								placeholder="Nhập tên sản phẩm" required="required"
								value="<%ServletContext context1 = request.getServletContext();
out.print(context1.getAttribute("tenSanPham"));%>">
						</div>
					</div>

					<div class="form-group">
						<label class="col-lg-3 control-label">Danh Mục:</label>
						<div class="col-lg-8">
							<select class="form-control" name="maDanhMuc">
								<option value="1">Bariton</option>
								<option value="2">Concert</option>
								<option value="3">Soprano</option>
								<option value="4">Tenor</option>
								<option value="5">Khác</option>
								<option value="6">Hula</option>
								<option value="7">Rosen</option>
								<option value="8">Seven</option>
								<option value="9">Lanikai</option>
								<option value="10">Vines</option>
								<option value="11">Dưới 1 triệu</option>
								<option value="12">Từ 1 - 2 triệu</option>
								<option value="13">Trên 2 triệu</option>
							</select>
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">Thương hiệu:</label>
						<div class="col-lg-8">
							<select class="form-control" name="thuongHieu">
								<option value="6">Hula</option>
								<option value="7">Rosen</option>
								<option value="8">Seven</option>
								<option value="9">Lanikai</option>
								<option value="10">Vines</option>
							</select>
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">Khoảng giá:</label>
						<div class="col-lg-8">
							<select class="form-control" name="khoangGia">
								<option value="11">Dưới 1 triệu</option>
								<option value="12">Từ 1 - 2 triệu</option>
								<option value="13">Trên 2 triệu</option>
							</select>
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">Giá:</label>
						<div class="col-lg-8">
							<input class="form-control" type="text" name="gia"
								placeholder="Nhập giá sản phẩm" required="required"
								value="<%ServletContext context2 = request.getServletContext();
out.print(context2.getAttribute("gia"));%>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">Giá khuyến mãi:</label>
						<div class="col-lg-8">
							<input class="form-control" type="text" name="giaKhuyenMai"
								placeholder="Nhập giá KM sản phẩm" value="" required="required"
								value="<%ServletContext context3 = request.getServletContext();
out.print(context3.getAttribute("giaKhuyenMai"));%>">
						</div>
					</div>


					<div class="form-group">
						<label class="col-lg-3 control-label">Hình ảnh chính:</label>
						<div class="col-lg-8">
							<input class="form-control" type="file" name="img1"
								placeholder="Nhập địa chỉ hình ảnh" value="" required="required"
								value="<%ServletContext context4 = request.getServletContext();
out.print(context4.getAttribute("img1"));%>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">Hình ảnh chi tiết 1:</label>
						<div class="col-lg-8">
							<input class="form-control" type="file" name="img2"
								placeholder="Nhập địa chỉ hình ảnh" value="" required="required"
								value="<%ServletContext context5 = request.getServletContext();
out.print(context5.getAttribute("img2"));%>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">Hình ảnh chi tiết 2:</label>
						<div class="col-lg-8">
							<input class="form-control" type="file" name="img3"
								placeholder="Nhập địa chỉ hình ảnh" value="" required="required"
								value="<%ServletContext context6 = request.getServletContext();
out.print(context6.getAttribute("img3"));%>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">Số lượng:</label>
						<div class="col-lg-8">
							<input class="form-control" type="text" name="soLuong"
								placeholder="Nhập số lượng sản phẩm" required="required"
								value="<%ServletContext context7 = request.getServletContext();
out.print(context7.getAttribute("soLuong"));%>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">Số lượng nhập:</label>
						<div class="col-lg-8">
							<input class="form-control" type="text" name="soLuongNhap"
								placeholder="Nhập số lượng đã nhập" required="required"
								value="<%ServletContext context8 = request.getServletContext();
out.print(context8.getAttribute("soLuongNhap"));%>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label">Mô tả:</label>
						<div class="col-md-8">
							<input class="form-control" type="text" name="moTa"
								placeholder="Nhập mô tả về sản phẩm" required="required"
								value="<%ServletContext context9 = request.getServletContext();
out.print(context9.getAttribute("moTa"));%>">
						</div>
					</div>

					<div class="form-group">
						<label class="col-md-3 control-label">Thông tin giới
							thiệu:</label>
						<div class="col-md-8">
							<input class="form-control" type="text" name="information"
								placeholder="Nhập thông tin về sản phẩm" required="required"
								value="<%ServletContext context10 = request.getServletContext();
out.print(context10.getAttribute("information"));%>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label">Tình trạng:</label>
						<div class="col-md-8">
							<select class="form-control" name="active">
								<option value="0">Chưa kiểm duyệt</option>
								<option value="1">Đã kiểm duyệt</option>
							</select>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label"></label>
						<div class="col-md-8">
							<button type="submit" class="btn btn-primary">Sửa sản
								phẩm</button>
							<a href="tables_dynamic.jsp">
								<button type="button" class="btn btn-primary">Hủy</button>
							</a>
						</div>
					</div>
				</form>
			</div>
		</div>
	</div>
	<hr>
</body>
</html>
