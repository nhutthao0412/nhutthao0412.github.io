...
@WebServlet("/gentelella-master/production/AddProduct")
public class AddProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String stt = "";
		try {
			ResultSet rs = new ConnectToDatabase().selectData("select * from ProductGuitar");
			while (rs.next()) {
				String s = rs.getString(1);
				stt = Integer.parseInt(s) + 1 + "";
			}
			new ConnectToDatabase().excuteSql("insert into ProductGuitar values('" + stt + "','" + maSanPham + "','"
